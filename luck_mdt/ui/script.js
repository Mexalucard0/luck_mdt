const mdtApp = new Vue({
    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://luck_mdt/getWarrants', JSON.stringify({}));
                $("#warrants").addClass("nav-active");
            } else if (page == "Chat") {
                $.post('http://luck_mdt/getChat', JSON.stringify({}));
                $("#chat").addClass("nav-active");
            } else if (page == "Faturalar") {
                $.post('http://luck_mdt/getFaturalar', JSON.stringify({}));
                $("#faturalar").addClass("nav-active")
            } else if (page == "Submit Report") {
                $("#submit-report").addClass("nav-active");
            } else if (page == "Faturalar") {
                $.post('http://luck_mdt/faturalar', JSON.stringify({}));
                $("#faturalar").addClass("nav-active");
            }
            else if (page == "Hack") {
            $("#hack").addClass("nav-active");
        }
            
        },

        hackType(type) {
            mdtApp.home.type = type;         
        },

        closeMDT() {
            $.post('http://luck_mdt/close', JSON.stringify({}));
        },

        takePhoto() {
            $.post('http://luck_mdt/takeMDTPhoto', JSON.stringify({
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://luck_mdt/performOffenderSearch', JSON.stringify({
        ...
        ChatNew() {
            if (this.chat_new) {
                $.post('http://luck_mdt/chatNew', JSON.stringify({
                    msg: this.chat_new
                }));
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://luck_mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://luck_mdt/saveOffenderChanges', JSON.stringify({
        ...
        mobese() {
            $.post('http://luck_mdt/mobese', JSON.stringify({
            }));
            return;
        },
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://luck_mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://luck_mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://luck_mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://luck_mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://luck_mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://luck_mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://luck_mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        telkonum() {
            $.post('http://luck_mdt/hack2', JSON.stringify({
                    type: "telkonum",
                    twitter: this.home.tel
                }));
                this.home = {
                    name: "darkNet",
                    error: null,
        ...
        elektrikkes() {
            $.post('http://luck_mdt/hack', JSON.stringify({
                    type: "elektrikkes"
                }));
                this.home = {
                    name: "darkNet",
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://luck_mdt/submitNewWarrant', JSON.stringify({
        ...
        SubmitNewFatura() {
            var date = new Date();
            date.setDate(date.getDate() + 7);
            $.post('http://luck_mdt/submitNewFatura', JSON.stringify({
                name: this.faturalar_new.name,
                firstname: this.faturalar_new.firstname,
                surname: this.faturalar_new.surname,
                label: this.faturalar_new.label,
                amount: this.faturalar_new.amount,
                names: this.names,
        ...
        DeleteSelectedWarrant() {
            $.post('http://luck_mdt/deleteWarrant', JSON.stringify({
        ...
        DeleteFatura() {
            $.post('http://luck_mdt/deleteFatura', JSON.stringify({
                id: this.faturalar_selected.id,
            }));
            this.fatura = [];
            this.faturalar_selected = [];
            return;
        },
        OpenReportById(id) {
            $.post('http://luck_mdt/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://luck_mdt/sentencePlayer', JSON.stringify({
    ...
    computed: {
        ...
        filtered_faturalar() {
            return this.faturalar.filter(faturalar => {
                if (
                    faturalar.label.toLowerCase().search(this.faturalar_search.toLowerCase()) != -1
                    )
                    return faturalar;
            })
        },
        filtered_chat() {
            return this.chat.filter(chat => {
                if (
                    chat.sender.toLowerCase().search(this.chat_search.toLowerCase()) != -1
                    )
                    return chat;
            })
        }
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.offender_changes.varlik = mdtApp.offender_selected.varlik;
            ...
            } else if (event.data.type == "completedFaturalar") {
                mdtApp.changePage("Faturalar");
            } else if (event.data.type == "returnedReportDetails") {
                mdtApp.changePage("Search Reports");
                mdtApp.report_selected = event.data.details;
            }else if (event.data.type == "HomeE") {
                    mdtApp.home.type = 'home';
                    mdtApp.home.error = event.data.error;
                    mdtApp.changePage("Hack"); 
                } else if (event.data.type == "returnedFaturalar") {
                    mdtApp.faturalar = [];
                    mdtApp.faturalar = event.data.faturalar;
                    mdtApp.names = event.data.name;
                    mdtApp.name2 = event.data.name2;
                } else if (event.data.type == "returnedChat") {
                    mdtApp.chat = event.data.chat;
                } else if (event.data.type == "returnedFaturalar2") {
                    mdtApp.faturalar = []; 
                ...
                mdtApp.jobgrade = event.data.jobgrade;
                mdtApp.homepage.sabika = event.data.sabika;
                mdtApp.homepage.arananlar = event.data.arananlar;
...
function ClearActiveNavItems() {
    ...
    $("#faturalar").removeClass("nav-active");
    $("#submit-report").removeClass("nav-active");
    $("#hack").removeClass("nav-active");
}
